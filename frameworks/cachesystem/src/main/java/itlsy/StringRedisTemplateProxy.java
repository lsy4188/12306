...
import itlsy.core.CacheGetFilter;
import itlsy.core.CacheGetIfAbsent;
import itlsy.core.CacheLoader;
import itlsy.properties.RedisDistributedProperties;
import itlsy.util.CacheUtil;
import itlsy.util.FastJson2Util;
...
public class StringRedisTemplateProxy implements DistributedCache {
    ...
    public <T> T safeGet(String key, Class<T> clazz, CacheLoader<T> cacheLoader, long timeout, TimeUnit timeUnit,
        ...
        T result = get(key, clazz);
        if (!CacheUtil.isNullOrBlank(result)
                || Optional.ofNullable(cacheGetFilter).map(item->item.filter(key)).orElse(false)
                || Optional.ofNullable(bloomFilter).map(item->!item.contains(key)).orElse(false)){
            return result;
        }
